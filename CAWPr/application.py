...
def index():
    db=get_db()
    db.execute(
        'SELECT p.id, url, party, state, level, office, profession, created'
        ' FROM post p'
...
def create():
    ...
    if request.method=='POST':
        ...
        if error is not None:
            ...
            db.execute(
                'INSERT INTO post (url, party, state, level, office, profession,text)'
                ' VALUES (%s,%s,%s,%s,%s,%s,%s);',
                (url,entry['party'],entry['state'],entry['level'],entry['office'],entry['profession'], entry['text'])
            )
...
def get_post(post_id):
    db=get_db()
    db.execute(
        'SELECT p.id, url, party, state, level, office, profession, text'
        ' FROM post p'
        ' WHERE p.id=%s;',
        (post_id,)
    )
...
def update(post_id):
    ...
    if request.method=='POST':
        ...
        if error is not None:
            ...
            db=get_db()
            db.execute(
                'UPDATE post SET url=%s, party=%s, state=%s, level=%s, office=%s, profession=%s, text=%s'
                ' WHERE id=%s;',
                (url,party,state,level,office,profession,text,post_id)
            )
            return redirect(url_for('application.index'))
